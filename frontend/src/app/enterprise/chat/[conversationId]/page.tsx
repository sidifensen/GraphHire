...
export default function EnterpriseChatDetailPage() {
  ...
  const currentUserId = enterpriseAuthStore((state) => state.user?.id ?? null);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 flex flex-col gap-4 min-h-[70vh]">
      ...
      <div className="rounded-2xl border border-surface-mid bg-surface-lowest p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
          <input
            value={inviteTime}
            onChange={(event) => setInviteTime(event.target.value)}
            placeholder="面试时间 2026-05-10T15:00:00"
            className="h-10 rounded-xl border border-surface-mid px-3 text-sm bg-transparent md:col-span-2"
          />
          <input
            value={inviteLocation}
            onChange={(event) => setInviteLocation(event.target.value)}
            placeholder="面试地点"
            className="h-10 rounded-xl border border-surface-mid px-3 text-sm bg-transparent"
          />
          <input
            value={inviteRemark}
        ...
        <label className="h-10 px-4 rounded-xl border border-primary text-primary text-sm font-bold inline-flex items-center cursor-pointer w-fit">
