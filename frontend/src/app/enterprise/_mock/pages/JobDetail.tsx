  ...
  return (
    <div className="flex flex-col h-full bg-background relative pb-[80px]">
      ...
      <div className="fixed bottom-0 left-0 right-0 w-full bg-surface-container-lowest border-t border-surface-variant p-4 shadow-[0_-4px_16px_rgba(0,0,0,0.05)] z-50 flex gap-4 items-center justify-center max-w-[375px] md:max-w-3xl mx-auto pb-safe">
        <Link to={`/jobs/${job.id}/edit`} className="flex-1 h-12 rounded-lg border border-outline-variant text-on-surface font-label-md text-[15px] font-medium hover:bg-surface-container-low flex justify-center items-center transition-colors active:scale-[0.98]">
            修改职位
        </Link>
        <Link to={`/recommendations?jobId=${job.id}`} className="flex-[2] h-12 rounded-lg bg-primary text-on-primary font-label-md text-[15px] font-medium shadow-sm hover:opacity-90 transition-opacity active:scale-[0.98] flex items-center justify-center gap-2">
