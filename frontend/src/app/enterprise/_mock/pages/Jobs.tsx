...
export default function Jobs() {
  ...
  return (
    <div className="flex flex-col h-full bg-background antialiased">
      <div className="flex-1 overflow-y-auto flex flex-col pb-24 md:pb-8 w-full max-w-7xl mx-auto px-container-margin md:px-8">
        ...
        <div className="hidden md:flex flex-row justify-between items-end pt-8 pb-4 border-b border-surface-variant sticky top-0 bg-background z-10">
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <Link to="/jobs/create" className="bg-primary text-on-primary h-10 px-4 rounded-lg flex items-center justify-center gap-2 shadow-sm hover:bg-on-primary-fixed-variant transition-colors">
          ...
          {filteredJobs.map((job) => (
            ...
            >
              ...
              <div className="flex gap-3 mt-1 pt-4 border-t border-surface-variant">
                <Link to={`/jobs/${job.id}`} className="flex-[0.8] h-10 rounded-lg border border-outline-variant font-label-md text-label-md text-on-surface flex items-center justify-center gap-1.5 hover:bg-surface-container-low transition-colors active:scale-95">
                  详情
                </Link>
                {job.status !== '已关闭' ? (
                  <Link to={`/recommendations?jobId=${job.id}`} className="flex-1 h-10 rounded-lg bg-primary font-label-md text-label-md text-on-primary flex items-center justify-center gap-1.5 hover:bg-opacity-90 transition-all shadow-sm hover:shadow active:scale-95">
                    匹配候选人
                  </Link>
                ) : (
      ...
      <Link to="/jobs/create" className="fixed bottom-20 right-4 bg-primary text-on-primary h-14 w-14 rounded-full flex items-center justify-center shadow-[0_4px_12px_rgba(0,102,255,0.3)] z-40 active:scale-95 transition-transform md:hidden hover:bg-on-primary-fixed-variant">
