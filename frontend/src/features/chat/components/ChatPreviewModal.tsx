...
export function ChatPreviewModal({ previewUrl, previewFileName, previewKind, onClose }: ChatPreviewModalProps) {
  ...
  return createPortal(
    <div data-testid="chat-resume-preview-modal" className="fixed inset-0 z-[9999] bg-black/70 p-2 md:p-6">
      <div className="mx-auto flex h-full w-full max-w-6xl flex-col overflow-hidden rounded-3xl bg-white/95 shadow-2xl backdrop-blur-lg">
        <div className="flex items-center justify-between border-b border-white/70 bg-white/90 px-4 py-3">
          <p className="truncate text-sm font-bold text-on-surface">{previewFileName || '简历预览'}</p>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg bg-white/85 px-3 py-1 text-sm text-on-surface shadow-sm hover:bg-white"
          >
            关闭预览
          </button>
        </div>
