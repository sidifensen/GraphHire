...
export function ChatConversationListPanel({
  ...
  return (
    <aside
      ...
      className={`chat-frosted-list-panel rounded-none md:rounded-3xl bg-white/72 backdrop-blur-xl ring-1 ring-white/70 shadow-[0_18px_45px_rgba(15,23,42,0.10)] overflow-hidden ${mobileMode === 'detail' ? 'hidden md:block' : ''}`}
    >
      <div className="h-12 px-3 border-b border-white/70 flex items-center bg-white/50">
        <input
          value={conversationKeyword}
          onChange={(event) => onKeywordChange(event.target.value)}
          placeholder="搜索会话..."
          className="h-9 w-full rounded-xl bg-white/75 px-3 text-sm text-on-surface outline-none ring-1 ring-white/80 focus:ring-2 focus:ring-primary/30"
        />
      </div>
      <div className="h-full overflow-y-auto p-2.5 space-y-2.5">
        ...
        {!loadingList && filteredList.length === 0 ? (
          <div className="rounded-2xl bg-white/60 px-4 py-10 text-center text-sm text-on-surface-variant ring-1 ring-white/80">暂无会话</div>
        ) : null}
        ...
        {filteredList.map((item) => {
          ...
          return (
            <button
              ...
              className={`group relative w-full overflow-hidden rounded-2xl px-3 py-3 text-left transition-all duration-150 ${selected ? 'bg-white/92 shadow-[0_12px_30px_rgba(37,99,235,0.18)]' : 'bg-white/62 hover:bg-white/84 hover:shadow-[0_8px_24px_rgba(15,23,42,0.12)]'}`}
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="shrink-0 text-right">
                  ...
                  {item.unreadCount > 0 ? (
                    <span className="inline-flex mt-1 min-w-5 h-5 px-1 items-center justify-center rounded-full bg-primary text-white text-[10px] font-bold shadow-[0_4px_12px_rgba(37,99,235,0.4)]">
