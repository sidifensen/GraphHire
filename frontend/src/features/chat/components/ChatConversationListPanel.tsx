    ...
    >
      <div className="h-12 px-3 flex items-center bg-surface-container/70">
        <input
          ...
          placeholder="搜索会话..."
          className="h-9 w-full rounded-xl bg-surface-container px-3 text-sm text-on-surface outline-none ring-1 ring-outline-variant/55 focus:ring-2 focus:ring-primary/30 placeholder:text-on-surface-variant"
        />
      </div>
      <div className="chat-scrollbar h-full overflow-y-auto p-2.5 space-y-2.5">
        ...
        {!loadingList && filteredList.length === 0 ? (
          <div className="rounded-2xl bg-surface-container px-4 py-10 text-center text-sm text-on-surface-variant ring-1 ring-outline-variant/55">暂无会话</div>
        ) : null}
        ...
        {filteredList.map((item) => {
          ...
          return (
            <button
              ...
              className={`group relative w-full overflow-hidden rounded-2xl px-3 py-3 text-left transition-all duration-150 ${selected ? 'bg-surface-container shadow-[0_12px_30px_rgba(37,99,235,0.18)] dark:shadow-[0_12px_30px_rgba(0,0,0,0.32)]' : 'bg-surface-container-low hover:bg-surface-container hover:shadow-[0_8px_24px_rgba(15,23,42,0.12)] dark:hover:shadow-[0_8px_24px_rgba(0,0,0,0.25)]'}`}
            >
